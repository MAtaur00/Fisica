...
update_status ModulePhysics::PostUpdate()
{
	...
	if(App->input->GetKey(SDL_SCANCODE_3) == KEY_DOWN)
	{
		...
		for (int i = 0, j = 0; i < 37; ++i) {
			vec[i] = { PIXEL_TO_METERS(points[j++]), PIXEL_TO_METERS(points[j++]) };
		}
...
void ModulePhysics::CreateBox(int mouse_x, int mouse_y, int h, int w)
{
